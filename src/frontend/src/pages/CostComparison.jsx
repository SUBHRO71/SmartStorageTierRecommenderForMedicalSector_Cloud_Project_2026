...
const CostComparison = () => {
  ...
  if (!data) return <div className="p-8 text-center text-red-500">Error loading cost comparison.</div>;

  const baseline = data.policies.find(p => p.name === 'All Standard')?.cost || 0;
  const mlModel = data.policies.find(p => p.name === 'Our ML Model')?.cost || 0;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">Comparing the ML recommender against baseline policies over a {data.horizon}-month horizon.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="card p-6 bg-gradient-to-br from-navy-900 to-navy-800 text-white">
            ...
            <p className="text-sm text-gray-300 mt-1">${new Intl.NumberFormat('en').format(baseline - mlModel)} saved annually.</p>
          </div>
              ...
              {data.policies.map((policy, idx) => (
                <div key={idx} className="border-b border-gray-100 last:border-0 pb-3 last:pb-0">
                  <div className="flex justify-between items-center mb-1">
                    <span className={`font-medium ${policy.name === 'Our ML Model' ? 'text-navy-700 font-bold' : 'text-gray-700'}`}>
                      {policy.name}
                    </span>
